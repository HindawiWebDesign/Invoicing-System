		...
		if (!(document.cookie.indexOf(n) > -1 || window.navigator && window.navigator.CookiesOK)) {
			...
			var o, s = {
					...
					setCookie: function (e, t, n, i, o) {
						...
						var r = [e + "=" + t, "expires=" + s.toUTCString(), "path=" + o || "/", "secure"];
				...
				a = {
					options: {
						...
						path: "/",
						expiryDays: 365,
						markup: ['<div class="notification {{containerClasses}}">', '<div class="cookies">', '<a href="#null" data-cc-event="click:dismiss" target="_blank" class="btn_accept">{{options.dismiss}}</a>', '<p class="cc_message">{{options.message}} <a data-cc-if="options.link" target="{{ options.target }}" class="cc_more_info" href="{{options.link || "#null"}}">{{options.learnMore}}</a></p>', "</div>", "</div>"]
					},
					init: function () {
						var t = window[e];
